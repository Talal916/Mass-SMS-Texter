...
def createButtons():
    ...
    sendMessageBtn = tk.Button(window, text="Send Message", command=sendMessageButtonClicked)
...
def callMessageSender(messageText):
    for index, row in customerFileData.iterrows():
        convertedNumber = "+92"+row['Mobile']
        personalizedMessage = messageText.replace("NAME", row['Name'])
        print("Sending message: \n"+personalizedMessage+"\n to: "+convertedNumber)
        smsCommands.sendSms(modem, convertedNumber, personalizedMessage)

def askConfirmation():
    res = messagebox.askyesnocancel("Are you sure?", "Are you sure want to send this message to all your customers?")
    return res
